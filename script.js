...
// ==================== REPORT GENERATION FUNCTIONS (MODIFIED WITH CHARTS) ====================

// Generate report preview in HTML with all project information and charts
function generateReportPreview() {
    ...
    // Generate data table HTML (first 10 rows)
    let dataTableHTML = '<table class="report-table" style="width:100%; border-collapse:collapse;">';
    dataTableHTML += '<tr style="background:#3498db; color:white;"><th>No</th><th>Waktu</th><th>Drawdown</th></tr>';
    
    if (data.length > 0) {
        const displayData = data.slice(0, 10);
        displayData.forEach((item, index) => {
            const timeDisplay = convertFromBase(item.t, timeUnit, 'time').toFixed(2);
            const drawdownDisplay = convertFromBase(item.s, lengthUnit, 'length').toFixed(3);
            dataTableHTML += `<tr>
                <td>${index + 1}</td>
                <td>${timeDisplay} ${timeUnit}</td>
                <td>${drawdownDisplay} ${lengthUnit}</td>
            </tr>`;
        });
        if (data.length > 10) {
            dataTableHTML += `<tr><td colspan="3" style="text-align:center;">... dan ${data.length - 10} data lainnya</td></tr>`;
        }
    } else {
        dataTableHTML += '<tr><td colspan="3" style="text-align:center;">Tidak ada data</td></tr>';
    }
    dataTableHTML += '</table>';
    
    // Create placeholder divs for charts
    const chartPlaceholders = `
        <div style="display: flex; flex-direction: column; gap: 20px; margin: 20px 0;">
            <div id="report-chart-drawdown-placeholder" style="width:100%; height:250px; background:#f5f5f5; border-radius:8px; display:flex; align-items:center; justify-content:center; color:#7f8c8d;">
                <p>📊 Grafik Drawdown vs Time (Log-Log)</p>
            </div>
            <div id="report-chart-diagnostic-placeholder" style="width:100%; height:250px; background:#f5f5f5; border-radius:8px; display:flex; align-items:center; justify-content:center; color:#7f8c8d;">
                <p>📈 Grafik Diagnostic (Semi-Log)</p>
            </div>
            <div id="report-chart-residual-placeholder" style="width:100%; height:200px; background:#f5f5f5; border-radius:8px; display:flex; align-items:center; justify-content:center; color:#7f8c8d;">
                <p>📉 Grafik Residual</p>
            </div>
        </div>
    `;
    ...
    const reportHTML = `
        ...
        <div class="report-section">
            <div class="report-section-title">D. DATA PENGUJIAN</div>
            ${dataTableHTML}
        </div>
        
        <div class="report-section">
            <div class="report-section-title">E. GRAFIK ANALISIS</div>
            ${chartPlaceholders}
        </div>
        
        <div class="report-section">
            <div class="report-section-title">F. HASIL ANALISIS PARAMETER AKUIFER</div>
            <table class="report-table">
        ...
        <div class="report-section">
            <div class="report-section-title">G. LOKASI DAN INFORMASI SUMUR</div>
        ...
        <div class="report-section">
            <div class="report-section-title">H. INTERPRETASI HIDROGEOLOGI</div>
        ...
        <div class="report-section">
            <div class="report-section-title">I. REKOMENDASI</div>
...
// Download PDF Report with map screenshot and charts
async function downloadPDF() {
    ...
    try {
        ...
        // Clone charts to temporary container
        const drawdownCanvas = document.getElementById('drawdownChart');
        const diagnosticCanvas = document.getElementById('diagnosticChart');
        const residualCanvas = document.getElementById('residualChart');
        
        if (drawdownCanvas) {
            const drawdownClone = drawdownCanvas.cloneNode(true);
            drawdownClone.width = 760;
            drawdownClone.height = 250;
            drawdownClone.style.width = '100%';
            drawdownClone.style.height = 'auto';
            drawdownClone.style.marginBottom = '10px';
            tempContainer.appendChild(drawdownClone);
            
            // Add title
            const title1 = document.createElement('h4');
            title1.textContent = 'Grafik Drawdown vs Time (Log-Log)';
            title1.style.margin = '10px 0 5px 0';
            title1.style.color = '#2c3e50';
            tempContainer.insertBefore(title1, drawdownClone);
        }
        
        if (diagnosticCanvas) {
            const diagnosticClone = diagnosticCanvas.cloneNode(true);
            diagnosticClone.width = 760;
            diagnosticClone.height = 250;
            diagnosticClone.style.width = '100%';
            diagnosticClone.style.height = 'auto';
            diagnosticClone.style.marginBottom = '10px';
            tempContainer.appendChild(diagnosticClone);
            
            // Add title
            const title2 = document.createElement('h4');
            title2.textContent = 'Grafik Diagnostic (Semi-Log)';
            title2.style.margin = '10px 0 5px 0';
            title2.style.color = '#2c3e50';
            tempContainer.insertBefore(title2, diagnosticClone);
        }
        
        if (residualCanvas) {
            const residualClone = residualCanvas.cloneNode(true);
            residualClone.width = 760;
            residualClone.height = 200;
            residualClone.style.width = '100%';
            residualClone.style.height = 'auto';
            residualClone.style.marginBottom = '10px';
            tempContainer.appendChild(residualClone);
            
            // Add title
            const title3 = document.createElement('h4');
            title3.textContent = 'Grafik Residual';
            title3.style.margin = '10px 0 5px 0';
            title3.style.color = '#2c3e50';
            tempContainer.insertBefore(title3, residualClone);
        }
        ...
        let chartsImageData = null;
        if (tempContainer.children.length > 1) { // Has charts
            const chartsCanvas = await html2canvas(tempContainer, {
                scale: 2,
            ...
            chartsImageData = chartsCanvas.toDataURL('image/png');
        }
        ...
        if (chartsImageData) {
            // Create a combined charts image
            const chartsImg = document.createElement('img');
            chartsImg.src = chartsImageData;
            chartsImg.style.width = '100%';
            chartsImg.style.height = 'auto';
            chartsImg.style.borderRadius = '8px';
            chartsImg.style.border = '1px solid #e0e0e0';
            chartsImg.style.marginBottom = '20px';
            
            // Find the chart placeholders section and replace
            const chartPlaceholders = document.getElementById('report-chart-drawdown-placeholder')?.parentNode;
            if (chartPlaceholders) {
                // Clear the placeholders
                while (chartPlaceholders.firstChild) {
                    chartPlaceholders.removeChild(chartPlaceholders.firstChild);
                }
                // Add the charts image
                chartPlaceholders.appendChild(chartsImg);
            }
        }
